...
function finishSignal(name,next){if(signalSolved.has(name))return;signalSolved.add(name);document.querySelectorAll('#signalProgress span').forEach((s,i)=>s.classList.toggle('done',signalSolved.has(['transformer','bdh','hrm'][i])));if(signalSolved.size===3)document.querySelector('#signalBlindChallenge')?.classList.remove('hidden');else if(next)setTimeout(()=>activateSignalRound(next),550);}
...
let depthStep=0,depthHalted=false,depthAwaitingDecision=false;let depthSegment=0;const maxDepthSteps=7;
function renderDepth(){
  ...
  else if(depthStep%3===0){depthAwaitingDecision=true;document.querySelector('#goDeeper').disabled=true;f.textContent=`Step ${depthStep}: Transformer ${depthStep>4?'has completed its fixed stack':'crossed another stacked transformation'}; BDH evolved to state ${state}; HRM completed fast L updates and changed H.`;document.querySelector('#haltPanel')?.classList.remove('hidden');}
  else f.textContent=`Step ${depthStep}: Transformer ${depthStep>4?'has completed its fixed stack':'advances through stacked blocks'}; BDH evolves state; HRM performs another fast L update under slower H.`;
...
document.querySelector('#goDeeper')?.addEventListener('click',()=>{if(depthStep<maxDepthSteps&&!depthHalted&&!depthAwaitingDecision){depthStep++;renderDepth();}});
document.querySelector('#haltChoice')?.addEventListener('click',()=>{const f=document.querySelector('#depthFeedback');depthHalted=true;depthAwaitingDecision=false;document.querySelector('#goDeeper').disabled=true;f.textContent='HALTED — computation stopped at step '+depthStep+'. Choose CONTINUE to resume, or RESET DEPTH to explore again.';document.querySelector('#haltPanel')?.classList.remove('hidden');});
document.querySelector('#continueChoice')?.addEventListener('click',()=>{depthSegment++;depthHalted=false;depthAwaitingDecision=false;document.querySelector('#goDeeper').disabled=depthStep>=maxDepthSteps;const f=document.querySelector('#depthFeedback');f.textContent='CONTINUE — the next recurrent segment is ready. Click GO DEEPER to advance.';document.querySelector('#haltPanel')?.classList.add('hidden');});
document.querySelector('#resetDepth')?.addEventListener('click',()=>{depthStep=0;depthSegment=0;depthHalted=false;depthAwaitingDecision=false;document.querySelector('#goDeeper').disabled=false;['haltPanel','depthSummary','depthTakeaway'].forEach(id=>document.getElementById(id)?.classList.add('hidden'));renderDepth();});
renderDepth();
...
let reconstructionI=0,draggedComponent=null,reconstructionPending=false;const placedComponents=new Set();
...
function renderRebuild(){draggedComponent=null;reconstructionPending=false;document.querySelector('#checkRebuild').disabled=false;document.querySelector('#clearRebuild').disabled=false;placedComponents.clear();document.querySelector('#placedComponents').replaceChildren();document.querySelectorAll('[data-component]').forEach(b=>{b.classList.remove('used','selected');b.draggable=true;});document.querySelector('#rebuildIndex').textContent=`RECONSTRUCTION ${String.fromCharCode(65+reconstructionI)} / 3`;document.querySelector('#rebuildFeedback').textContent='';}
...
const rebuildCanvas=document.querySelector('#rebuildCanvas');rebuildCanvas?.addEventListener('dragover',e=>e.preventDefault());rebuildCanvas?.addEventListener('drop',e=>{e.preventDefault();addComponent();});rebuildCanvas?.addEventListener('click',e=>{if(!e.target.closest('#placedComponents button'))addComponent();});
function addComponent(){if(reconstructionPending||!draggedComponent||draggedComponent.classList.contains('used'))return;const c=draggedComponent.dataset.component;placedComponents.add(c);draggedComponent.classList.add('used');draggedComponent.classList.remove('selected');const chip=document.createElement('button');chip.textContent=c;chip.type='button';chip.addEventListener('click',()=>{if(reconstructionPending)return;placedComponents.delete(c);draggedComponent=null;document.querySelector(`[data-component="${CSS.escape(c)}"]`)?.classList.remove('used');chip.remove();});document.querySelector('#placedComponents').append(chip);draggedComponent=null;}
document.querySelector('#clearRebuild')?.addEventListener('click',()=>{if(!reconstructionPending)renderRebuild();});
document.querySelector('#checkRebuild')?.addEventListener('click',()=>{
  if(reconstructionPending)return;const r=reconstructions[reconstructionI],f=document.querySelector('#rebuildFeedback');const required=new Set(r.required);const missing=r.required.filter(x=>!placedComponents.has(x));const extra=[...placedComponents].filter(x=>!required.has(x));
  ...
  reconstructionPending=true;document.querySelector('#checkRebuild').disabled=true;document.querySelector('#clearRebuild').disabled=true;f.innerHTML=`<strong>${r.name}</strong> — ${r.diagram}`;f.className='feedback good';
...
function placeArchitecture(drop){if(!draggedArch)return;const name=draggedArch.dataset.archLabel;Object.keys(classifications).forEach(k=>{if(classifications[k]===name)delete classifications[k];});classifications[drop.dataset.classifyDrop]=name;document.querySelectorAll('[data-classify-drop]').forEach(target=>{target.querySelectorAll('strong').forEach(x=>x.remove());const assigned=classifications[target.dataset.classifyDrop];if(assigned){const tag=document.createElement('strong');tag.textContent=assigned;target.append(tag);}});['evidencePhase','impostorPhase','chapterReveal'].forEach(id=>document.getElementById(id)?.classList.add('hidden'));document.querySelectorAll('[data-arch-label]').forEach(x=>x.classList.remove('selected'));draggedArch=null;const f=document.querySelector('#classificationFeedback');f.textContent=Object.keys(classifications).length+' / 3 machines labeled.';f.className='feedback';if(Object.keys(classifications).length===3){const ok=classifications.alpha==='Transformer'&&classifications.beta==='BDH'&&classifications.gamma==='HRM';f.textContent=ok?'Classification hypothesis is correct. Now justify every choice with evidence.':'At least one label conflicts with the observed structure. Recheck the three experiments.';f.className='feedback '+(ok?'good':'bad');if(ok){renderEvidenceCases();document.querySelector('#evidencePhase')?.classList.remove('hidden');}}}
...
function renderEvidenceCases(){document.querySelector('#evidenceFeedback').textContent='';document.querySelector('#impostorFeedback').textContent='';document.querySelectorAll('.impostor-clues button').forEach(b=>b.classList.remove('correct','wrong'));const wrap=document.querySelector('#evidenceCases');wrap.replaceChildren();Object.entries(reasonBank).forEach(([key,reasons])=>{const article=document.createElement('article');article.dataset.evidenceMachine=key;article.innerHTML=`<small>MACHINE ${key==='alpha'?'α':key==='beta'?'β':'γ'} — ${classifications[key].toUpperCase()}</small>`;const choices=document.createElement('div');choices.className='evidence-choices';reasons.forEach(([text,correct])=>{const b=document.createElement('button');b.textContent=text;b.dataset.correct=correct?'true':'false';b.addEventListener('click',()=>{b.classList.toggle('selected');document.querySelector('#evidenceFeedback').textContent='Evidence changed. Check your choices again.';['impostorPhase','chapterReveal'].forEach(id=>document.getElementById(id)?.classList.add('hidden'));});choices.append(b);});article.append(choices);wrap.append(article);});}
document.querySelector('#checkEvidence')?.addEventListener('click',()=>{const cases=[...document.querySelectorAll('[data-evidence-machine]')];const ok=cases.length===3&&cases.every(article=>{const selected=[...article.querySelectorAll('button.selected')];return selected.length===2&&selected.every(b=>b.dataset.correct==='true');});const f=document.querySelector('#evidenceFeedback');f.textContent=ok?'Accepted — each architecture was identified from multiple structural clues, not a single buzzword.':'Each machine needs exactly two structural reasons, and both must be architecture evidence.';f.className='feedback '+(ok?'good':'bad');if(ok)document.querySelector('#impostorPhase')?.classList.remove('hidden');});
